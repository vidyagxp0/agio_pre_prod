    ...
    <div class="process-groups">
        <div class="scope-bar">
            <button
                ...
                onclick="openEffectivenessPrint()"
            >
                <i class="fa fa-print" aria-hidden="true"></i>
                Print
            </button>
        </div>
        <div class="active" onclick="openTab('internal-audit', this)">Effectiveness-Check</div>
                    ...
                    <div class="mt-1 mb-2 bg-white" style="height: auto; padding: 10px;">
                        <div class="d-flex align-items-center">
                            ...
                            <div class="filter-bar d-flex flex-wrap justify-content-between">
                                    ...
                                    <div class="filter-item">
                                        <label for="date_from_effectiveness">Date From</label>
                                        <input type="date" class="custom-select form-control" id="date_from_effectiveness">
                                    </div>
                                    <div class="filter-item">
                                        <label for="date_to_effectiveness">Date To</label>
                                        <input type="date" class="custom-select form-control" id="date_to_effectiveness">
                                    </div>
                            ...
                            <table class="table table-bordered" style="width: 120%;">
                                <thead>
                                    <tr>
                                            ...
                                            <th rowspan="2">Effectiveness check Plan </th>
                                ...
                                <tbody id="tableData">
                                    @include('frontend.forms.Logs.filterData.effectivenes_data')
                                </tbody>
                            </table>
...
<script>
    ...
    const filterData = {
        effectivenes_department: null,
        div_ideffectiveness: null,
        period_lab: null,
        dateeffectivenessFrom: null,
        dateEffectivenessTo: null,
        // categoryofcomplaints: null
    };

    $('#initiator_group').change(function() {
        filterData.effectivenes_department = $(this).val();
        filterRecords();
    });

    $('#division_id').change(function() {
        filterData.div_ideffectiveness = $(this).val();
        filterRecords();
    });
    ...
    $('#date_from_effectiveness').change(function() {
        filterData.dateeffectivenessFrom = $(this).val();
        filterRecords();
    });

    $('#date_to_effectiveness').change(function() {
        filterData.dateEffectivenessTo = $(this).val();
        filterRecords();
    });
    ...
    async function filterRecords() {
        ...
        try {
            const postUrl = "{{ route('api.effectiveness.filter') }}";
...
<script>
    window.openEffectivenessPrint = function () {
        const baseUrl = @json(
            route(
                'rcms.logs.print',
                ['slug' => 'effectiveness-check']
            )
        );
        ...
        const dateFromElement =
            document.getElementById('date_from_effectiveness');

        const dateToElement =
            document.getElementById('date_to_effectiveness');
