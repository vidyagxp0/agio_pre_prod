                ...
                <div class="inner-block">
                    <div class="main-head">
                        <div class="default-name">
                            {{ Helpers::getDivisionName(session()->get('division')) }}/LI/{{ date('Y') }}/{{ str_pad($document->record, 4, '0', STR_PAD_LEFT) }}
                        </div>

                            <div class="btn-group">
                                <a href="{{ url('rcms/marketAuditReport',$document->id)}}"> <button type="button">Print</button></a>
                            </div>
                    </div>
                    <div class="info-list">
                        <div class="list-item">
                                ...
                                @foreach ($audit as $audits)
                                    <tr>
                                        <td class="viewdetails"><a
                                                href="{{ route('marketcomplaint.marketauditDetails', $audits->id) }}">{{ $audits->activity_type }}</a>
                                        </td>
