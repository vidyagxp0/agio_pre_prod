            ...
            <div class="card">
                ...
                <div class="card-body">
                    <table id="example1" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>Document Language Code</th>
                                <th>Document Language Name</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($language as $department)
                                <tr>
                                    <td>{{ $department->lcode }}</td>

                                    <td>{{ $department->lname }}</td>
                                    <td>
                                        <a class="mdi mdi-table-edit"
                                            href="{{ route('documentlanguage.edit', $department->id) }}"><button
                                                class="btn btn-dark">Edit</button></a>

                                        <form action="{{ route('documentlanguage.destroy', $department->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">Create Document Language</h4>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form action="{{ route('documentlanguage.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="col-md-12">
