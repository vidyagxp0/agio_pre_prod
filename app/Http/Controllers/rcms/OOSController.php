...
class OOSController extends Controller
{
    ...
    public static function show($id)
    {
        ...
        $oos_details = $data->grids()
            ->where('identifier', 'oos_detail')
            ->first();

        $oos_detail = [];

        if ($oos_details && $oos_details->data) {
            $oos_detail = $oos_details->data ?? [];
        }
                                                ...
                                                function ($message) use ($email, $changestage) {
                                                    $message->to($email)
                                                            ->subject(
                                                                "Agio Notification: OOS/OOT, Record #"
                                                                . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                                . " - Activity: HOD Primary Review Complete"
                                                            );
                                                }
                                            );
                                        } catch (\Throwable $e) {
                                    ...
                                    function ($message) use ($email, $changestage) {
                                        $message->to($email)
                                                ->subject(
                                                    "Agio Notification: OOS/OOT, Record #"
                                                    . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                    . " - Activity: Phase IA HOD Review Complete"
                                                );
                                    }
                                );
                            } catch (\Throwable $e) {
                                    ...
                                    function ($message) use ($email, $changestage) {
                                        $message->to($email)
                                                ->subject(
                                                    "Agio Notification: OOS/OOT, Record #"
                                                    . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                    . " - Activity: Phase IA QA/CQA Review Complete"
                                                );
                                    }
                                );
                            } catch (\Throwable $e) {
                                                ...
                                                function ($message) use ($email, $changestage) {
                                                    $message->to($email)
                                                            ->subject(
                                                                "Agio Notification: OOS/OOT, Record #"
                                                                . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                                . " - Activity: Phase IB Investigation"
                                                            );
                                                }
                                            );
                                        } catch (\Throwable $e) {
                                ...
                                function ($message) use ($email, $changestage) {
                                    $message->to($email)
                                            ->subject(
                                                "Agio Notification: OOS/OOT, Record #"
                                                . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                . " - Activity: Phase IB HOD Review Complete"
                                            );
                                }
                            );
                        } catch (\Throwable $e) {
                                    ...
                                    function ($message) use ($email, $changestage) {
                                        $message->to($email)
                                                ->subject(
                                                    "Agio Notification: OOS/OOT, Record #"
                                                    . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                    . " - Activity: Phase IB QA Review Complete"
                                                );
                                    }
                                );
                            } catch (\Throwable $e) {
                                    ...
                                    function ($message) use ($email, $changestage) {
                                        $message->to($email)
                                                ->subject(
                                                    "Agio Notification: OOS/OOT, Record #"
                                                    . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                    . " - Activity: P I B Assignable Cause Not Found"
                                                );
                                    }
                                );
                            } catch (\Throwable $e) {
                                    ...
                                    function ($message) use ($email, $changestage) {
                                        $message->to($email)
                                                ->subject(
                                                    "Agio Notification: OOS/OOT, Record #"
                                                    . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                    . " - Activity: Phase II A HOD Review Complete"
                                                );
                                    }
                                );
                            } catch (\Throwable $e) {
                                    ...
                                    function ($message) use ($email, $changestage) {
                                        $message->to($email)
                                                ->subject(
                                                    "Agio Notification: OOS/OOT, Record #"
                                                    . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                    . " - Activity: Phase II A CQA/QA Review Complete"
                                                );
                                    }
                                );
                            } catch (\Throwable $e) {
                                    ...
                                    function ($message) use ($email, $changestage) {
                                        $message->to($email)
                                                ->subject(
                                                    "Agio Notification: OOS/OOT, Record #"
                                                    . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                    . " - Activity: P II A Assignable Cause Not Found"
                                                );
                                    }
                                );
                            } catch (\Throwable $e) {
            ...
            if ($changestage->stage == 20) {
               if (!$changestage->reopen_approval_comments_uaa) {
                ...
                $changestage->P_II_B_Assignable_Cause_Not_Found_Comment = $request->comment;
                    ...
                    // Merge all & remove duplicate users
                    $users = collect($initiatorList)
                        ->merge($hodList)
                        ->merge($qaList)
                        ->merge($cqaList)
                        ->unique('user_id')
                    ...
                    try {
                        Mail::send(
                            ...
                            [
                                'data'    => $changestage,
                                'site'    => "OOS/OOT",
                                'history' => "P II B Assignable Cause Not Found",
                                'process' => 'OOS/OOT',
                                'comment' => $request->comment,
                                'user'    => Auth::user()->name
                            ],
                            function ($message) use ($email, $changestage) {
                                $message->to($email)
                                        ->subject(
                                            "Agio Notification: OOS/OOT, Record #"
                                            . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                            . " - Activity: P II B Assignable Cause Not Found"
                                        );
                            }
                        );
                    } catch (\Throwable $e) {
                                        ...
                                        try {
                                            Mail::send(
                                                ...
                                                [
                                                    'data'    => $changestage,
                                                    'site'    => "OOS/OOT",
                                                    'history' => "P-IB Assignable Cause Found",
                                                    'process' => 'OOS/OOT',
                                                    'comment' => $request->comment,
                                                    'user'    => Auth::user()->name
                                                ],
                                                function ($message) use ($email, $changestage) {
                                                    $message->to($email)
                                                            ->subject(
                                                                "Agio Notification: OOS/OOT, Record #"
                                                                . str_pad($changestage->record, 4, '0', STR_PAD_LEFT)
                                                                . " - Activity: P-IB Assignable Cause Found"
                                                            );
                                                }
                                            );
                                        } catch (\Throwable $e) {
