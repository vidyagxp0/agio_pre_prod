...
foreach ($pre as $processName => $modelClass) {
 ...
 if (!empty($request->final_attach) || !empty($request->deleted_Approval_Attachments)) {
        ...
        if ($lastopenState->related_records != $openState->related_records ) {
            ...
            $history->cc_id = $id;
            $history->activity_type = 'Action Item Related Records';
            // $previous = is_array($lastopenState->related_records) ? implode(', ', $lastopenState->related_records) : $lastopenState->related_records;
            // $current = is_array($openState->related_records) ? implode(', ', $openState->related_records) : $openState->related_records;
        ...
        if (strtolower($request->username) == strtolower(Auth::user()->emp_code) && Hash::check($request->password, Auth::user()->password)) {
            ...
            if (empty($changeControl->related_records) || empty($changeControl->description) || empty($changeControl->assign_to) || empty($changeControl->hod_preson)) 
                ...
                if (!empty($changeControl->assign_to)) {
                    ...
                    $assignedEmail = Helpers::getUserEmail($changeControl->assign_to);
...
public function lastStage(Request $request, $id){
    if (strtolower($request->username) == strtolower(Auth::user()->emp_code) && Hash::check($request->password, Auth::user()->password)) {
        ...
        if ($changeControl->stage == 4) {
            ...
            $assignedUser = collect();

            if (!empty($changeControl->assign_to)) {
                $assignedUser = collect([
                    (object) ['user_id' => $changeControl->assign_to]
                ]);
            }
            $usersmerge = collect()
    ...
    if (strtolower($request->username) == strtolower(Auth::user()->emp_code) && Hash::check($request->password, Auth::user()->password)) {
        ...
        if ($changeControl->stage == 1) {
            ...
            $history->save();
                ...
                $initiatorList = Helpers::getInitiatorUserList($changeControl->division_id);
