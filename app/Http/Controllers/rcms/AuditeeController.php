        ...
        if (!empty($request->file_attachment_guideline)) {
            ...
            if ($request->hasfile('file_attachment_guideline')) {
                foreach ($request->file('file_attachment_guideline') as $file) {
                    $name = $request->name . 'file_attachment_guideline' .date('d-m-Y_H-i-s') . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
                    $file->move('upload/', $name);
                    $files[] = $name;
                }
            }
        ...
        if (!empty($request->file_attachment)) {
            ...
            if ($request->hasfile('file_attachment')) {
                foreach ($request->file('file_attachment') as $file) {
                    $name = $request->name . 'file_attachment' .date('d-m-Y_H-i-s') . '_' . uniqid() . '.' . $file->getClientOriginalExtension();
                    $file->move('upload/', $name);
                    $files[] = $name;
                }
            }
...
$Cft = ExternalAuditCFT::where('external_audit_id', $id)->first();
 ...
 ///////////////////////////////////////////////////////////////////////////////////////////////////////////
     ...
     public function summery_pdf($id)
    {
        ...
        if (!$data) {
            abort(404, 'External Audits record not found.');
        }
        ...
        */
        $time = Carbon::now();
        $pdf = PDF::loadView(
            'frontend.externalAudit.External_Audit_summery_pdf',
            compact(
                'data',
                'actionItems',
                'time'
        ...
        */
        ...
        return $pdf->stream(
            'External_Audit_Action_Item_Summary_' . $data->id . '.pdf'
        );
    }
