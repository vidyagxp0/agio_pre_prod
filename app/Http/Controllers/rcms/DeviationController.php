...
class DeviationController extends Controller
{
    ...
    public function deviation_child_1(Request $request, $id)
    {
        ...
        if ($request->child_type == "capa") {
            ...
            return view('frontend.forms.capa', compact('parent_id','relatedRecords','record_number','record', 'parent_record','parent_type', 'record',  'parent_short_description', 'parent_initiator_id', 'parent_intiation_date', 'parent_name','reference_record', 'parent_division_id', 'parent_record', 'old_records', 'cft','parent_due_date','parent_division_id'));
                        ...
                        // if($u->q_m_s_divisions_id == $deviation->division_id){
                            $email = Helpers::getUserEmail($u->user_id);
                                ...
                                try {
                                    Mail::send(
                                        'mail.view-mail',
                                        ['data' => $deviation, 'site'=>"DEV", 'history' => "More Information Required", 'process' => 'Deviation', 'comment' => $request->comment, 'user'=> Auth::user()->name],
                        ...
                        // if($u->q_m_s_divisions_id == $deviation->division_id){
                            $email = Helpers::getUserEmail($u->user_id);
                                ...
                                try {
                                    Mail::send(
                                        'mail.view-mail',
                                        ['data' => $deviation, 'site'=>"DEV", 'history' => "Cancel", 'process' => 'Deviation', 'comment' => $deviation->cancelled_comment, 'user'=> Auth::user()->name],
                                        function ($message) use ($email, $deviation) {
                                            $message->to($email)
                        ...
                        // if($u->q_m_s_divisions_id == $deviation->division_id){
                            $email = Helpers::getUserEmail($u->user_id);
                                ...
                                $data = [
                                    'data'    => $deviation,
                                    'site'    => "Deviation",
                                    'history' => "Send to QA/CQA Initial Review",
                                    'process' => 'Deviation',
                                    'comment' => $request->comment,
                                    'user'    => Auth::user()->name
                                ];
                            ...
                            $email = Helpers::getUserEmail($u->user_id);
                               ...
                               try {
                                ...
                                $data = [
                                    'data'    => $deviation,
                                    'site'    => "Deviation",
                                    'history' => "Send to Pending Initiator Update",
                                    'process' => 'Deviation',
                                    'comment' => $request->comment,
                                    'user'    => Auth::user()->name
                                ];
            ...
            if($request->username == Auth::user()->emp_code && Hash::check($request->password, Auth::user()->password)) {
                                                ...
                                                // if($u->q_m_s_divisions_id == $deviation->division_id){
                                                    $email = Helpers::getUserEmail($u->user_id);
                                                        ...
                                                        $data = [
                                                            'data'    => $deviation,
                                                            'site'    => "Deviation",
                                                            'history' => "QA/CQA Initial Review Complete",
                                                            'process' => 'Deviation',
                                                            'comment' => $request->comment,
                                                            'user'    => Auth::user()->name
                                                        ];
                ...
                if ($deviation->stage == 5) {
                    ...
                    foreach ($extensionchild as $ext) {
                        ...
                        if (!in_array($extensionchildStatus, ['closed - done', 'closed - reject', 'closed cancelled'])) {
                            $hasPending4 = true;
                            break;
                        }
                    }
